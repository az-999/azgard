...
void h_mqtt()//mqtt
{
  ...
  message += F("<input type=\"text\" name=\"mqttTopicPref\" maxlength="); message += String(maxStrParamLength) + F(" value=\"");
  message += String(mqttTopicPref) + F("\" /> Префикс всех топиков<br/>\n");

  
  message += F("<input type = \"checkbox\" name=\"addUserTopic\" id=\"addUserTopic\" ");
  if (addUserTopic == true) message += F(" checked=\"checked\"");
  message += F("onchange=\"openUrl('/switch?addUserTopic=' + this.checked);\" ");
  message += F("<label for=\"addUserTopic\"> добавлять в топик имя пользователя</label>");
  message += F("<p>");
...
/*void h_mail()//mail
{
  ...
  message += F("<html>\n");
  message += F("<head>\n");
  message += F("<title>Настройка отправки почты</title>\n");
  message += F("<meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">\n");
  message += F("<meta charset=\"utf-8\">\n");
  message += F("<style type=\"text/css\">");
  message += F(".adressfild {width: 35px; text-align: center;}");
  ...
  message += F("</head>\n");
  message += F("<body>\n");
  message += F("<form name=\"mail\" method=\"get\" action=\"/save\">\n");
  message += F("<h3>Настройка Mail</h3>\n");
  message += F("<br />");
  
  message += F("<input type = \"checkbox\" name=\"useMail\" id=\"useMail\" ");
  if (useMail == true) message += F(" checked=\"checked\"");
  message += F("onchange=\"openUrl('/switch?useMail=' + this.checked);\" ");
  message += F("<label for=\"useMail\"> включить отправку почты</label>");
  message += F("<br />");

  message += F("<input type = \"checkbox\" name=\"useMailSwt1\" id=\"useMailSwt1\" ");
  if (useMailSwt1 == true) message += F(" checked=\"checked\"");
  message += F("onchange=\"openUrl('/switch?useMailSwt1=' + this.checked);\" ");
  message += F("<label for=\"useMailSwt1\"> включить для Реле 1</label>");
  message += F("<br />");

  message += F("<input type = \"checkbox\" name=\"useMailSwt2\" id=\"useMailSwt2\" ");
  if (useMailSwt2 == true) message += F(" checked=\"checked\"");
  message += F("onchange=\"openUrl('/switch?useMailSwt2=' + this.checked);\" ");
  message += F("<label for=\"useMailSwt2\"> включить для Реле 2</label>");
  message += F("<br />");

  message += F("<input type = \"checkbox\" name=\"useMailSwt3\" id=\"useMailSwt3\" ");
  if (useMailSwt3 == true) message += F(" checked=\"checked\"");
  message += F("onchange=\"openUrl('/switch?useMailSwt3=' + this.checked);\" ");
  message += F("<label for=\"useMailSwt3\"> включить для Реле 3</label>");
  message += F("<br />");

  message += F("<input type = \"checkbox\" name=\"useMailSwt4\" id=\"useMailSwt4\" ");
  if (useMailSwt4 == true) message += F(" checked=\"checked\"");
  message += F("onchange=\"openUrl('/switch?useMailSwt4=' + this.checked);\" ");
  message += F("<label for=\"useMailSwt4\"> включить для Реле 4</label>");
  message += F("<p>");
  
  message += F("<input type=\"text\" name=\"smtpHost\" maxlength="); message += String(maxStrParamLength) + F(" value=\"");
  message += String(smtpHost) + F("\" /> smtp хост<br/>\n");
  message += F("<input type=\"text\" name=\"smtpPort\" maxlength="); message += String(maxStrParamLength) + F(" value=\"");
  message += String(smtpPort) + F("\" /> smtp порт<br/>");
  message += F("<input type=\"text\" name=\"smtpUser\" maxlength="); message += String(maxStrParamLength) + F(" value=\"");
  message += String(smtpUser) + F("\" /> smtp пользователь<br/>");
  message += F("<input type=\"text\" name=\"smtpPass\" maxlength="); message += String(maxStrParamLength) + F(" value=\"");
  message += String(smtpPass) + F("\" /> smtp пароль<br/>");
  message += F("<input type=\"text\" name=\"mailTo\" maxlength="); message += String(maxStrParamLength) + F(" value=\"");
  message += String(mailTo) + F("\" /> mailTo<br/>");
  message += F("<input type=\"text\" name=\"mailSubject\" maxlength="); message += String(maxStrParamLength) + F(" value=\"");
  message += String(mailSubject) + F("\" /> тема<br/>");

  message += F("<input type=\"button\" value=\"Проверить\" onclick=\"openUrl('/switch?mail_test=1');\">");
