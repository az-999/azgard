...
**Ollama на RTX 50 в Docker: GPU 0%, CPU 100%.** В логах контейнера Ollama пишет CUDA и VRAM, но в диспетчере задач Windows GPU не загружается — известная особенность WSL2 + Blackwell. **Обход:** гонять инференс через **Windows Ollama**. 1) В `docker-compose.yml` у сервиса `open-webui` в `environment` замени `OLLAMA_BASE_URL=http://ollama:11434` на `OLLAMA_BASE_URL=http://host.docker.internal:11434`. 2) Запусти стек без контейнера Ollama: `docker compose up -d whisper qwen3-tts open-webui` (Ollama в Windows должен быть запущен на 11434). 3) В Open WebUI выбери модель — запросы пойдут в Windows Ollama, загрузка GPU должна отображаться в диспетчере задач.  
...
**Voice Studio** (создание и сохранение профилей): http://127.0.0.1:5002/voice-studio — в настройках **TTS Server Base URI** = `http://localhost:8880`. Кнопка **«Generate»** во вкладке Voice Clone даёт 500: она шлёт запрос не на тот эндпоинт. Делайте так: **1)** Заполните имя (Alice), язык, образец аудио и транскрипт → нажмите **«Save clone profile»** (сохранит профиль). **2)** Перейдите во вкладку **Playground**, выберите сохранённый профиль (Alice), введите текст и нажмите воспроизведение — там используется `clone:…` и всё работает. Библиотека голосов сохраняется в volume и не пропадает при перезапуске. Чтобы озвучка в Open WebUI шла голосом Alice: в compose задано `AUDIO_TTS_VOICE=clone:Alice`; при необходимости смените голос в настройках Open WebUI (Settings → Audio) или поменяйте переменную на другой профиль (например `clone:Имя` или `Vivian`).  
...
http://127.0.0.1:5002/voice-studio
